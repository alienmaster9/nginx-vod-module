...
static ngx_str_t extract_token_from_query(ngx_http_request_t *r) {
    ngx_str_t token = ngx_null_string;

    ngx_str_t *args = &r->args;
    if (args->len > 0) {
        ngx_str_t token_param = ngx_string("token=");
        u_char *token_start = ngx_strstrn(args->data, token_param.data, token_param.len - 1);
        if (token_start) {
            token_start += token_param.len;
            u_char *token_end = ngx_strchr(token_start, '&');
            if (token_end == NULL) {
                token_end = args->data + args->len;
            }
            token.data = token_start;
            token.len = token_end - token_start;
        }
    }
    ...
    ngx_str_t* content_type)
{
    ...
    ngx_str_t token = extract_token_from_query(submodule_context->r);
    ...
    // Append the token to each segment URL in the playlist
    if (token.len > 0) {
        ngx_str_t token_prefix = ngx_string("?token=");
        ngx_str_t new_response;
        new_response.len = response->len + (token.len + token_prefix.len) * 10; // approximate size
        new_response.data = ngx_pnalloc(submodule_context->request_context.pool, new_response.len);
        if (new_response.data == NULL) {
            return NGX_ERROR;
        }

        u_char *p = new_response.data;
        u_char *last = response->data + response->len;
        u_char *cur = response->data;

        while (cur < last) {
            if (ngx_strncmp(cur, ts_file_ext, sizeof(ts_file_ext) - 1) == 0 ||
                ngx_strncmp(cur, m4s_file_ext, sizeof(m4s_file_ext) - 1) == 0) {
                p = ngx_copy(p, cur, sizeof(ts_file_ext) - 1);
                cur += sizeof(ts_file_ext) - 1;
                p = ngx_copy(p, token_prefix.data, token_prefix.len);
        ...
        new_response.len = p - new_response.data;
